...
static void _derive_keys(const char *keyfile_dir) {
    ...
    if (h_cfg.t210b01) {
        ...
        _save_keys_to_sd(keyfile_dir, keys, titlekey_buffer, is_dev, NULL, 0);
    } else {
        // On Erista, save both prod and dev key sets, each with its own new-gen keys.
        _save_keys_to_sd(keyfile_dir, &prod_keys, titlekey_buffer, false, &new_keys, new_gen_mkr);
        _key_count = 0;
        _save_keys_to_sd(keyfile_dir, &dev_keys, NULL, true, &new_keys_dev, new_gen_mkr);
    }
}
